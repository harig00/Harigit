function varargout=ANT_wantarcticaRM12(res,buf)  
...
if ~isstr(res) % Not a demo
  ...
  if res==0 && buf==0
    fnpl=fullfile(whereitsat,'ANT_wantarcticaRM12.mat');
  elseif buf==0;
    fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','ANT_wantarcticaRM12',res));
  elseif buf~=0
    fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','ANT_wantarcticaRM12',res,buf));
  end
  ...
  if exist(fnpl,'file')==2 
    ...
    if buf==0
        if res>10
          ...
          XY=ANT_wantarcticaRM12(10);
          XYb=bezier(XY,round(res/10));
          XY=XYb;
        else
          XY=ANT_wantarcticaRM12(0);
          XYb=bezier(XY,res);
          XY=XYb;
        end
    end
    ...
    if buf ~=0
      ...
      fnpl2=fullfile(whereitsat,sprintf('%s-%i-%g.mat','ANT_wantarcticaRM12',0,buf));
    ...
    % Get the original Pantarctica region
    % But remember to rotate to the equator
    XYp = ANT_pantarcticaRM12(10);
    ...
    XYp = [lon lat];
          ...
          % Now subtract 
          XY3 = ANT_eantarcticaRM12(10);

          % Now subtract these regions
          [x1,y1] = polybool('subtraction',LonB,LatB,XYp(:,1),XYp(:,2));
          [x2,y2] = polybool('subtraction',x1,y1,XY3(:,1),XY3(:,2));
