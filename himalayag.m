function varargout=himalayag(res,buf)  
% XY=HIMALAYAG(res,buf)
% 
% Returns the coordinates of the Himalaya region, which was
...
if ~isstr(res) % Not a demo
  ...
  if res==0 && buf==0
    fnpl=fullfile(whereitsat,'Himalayag.mat');
  elseif buf==0;
    fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','Himalayag',res));
  elseif buf~=0
    fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','Himalayag',res,buf));
  end
  ...
  if exist(fnpl,'file')==2 
    ...
    if buf==0
        if res==0
          % First part 
          load(fullfile(whereitsat,'Himalayag.mat'));
    ...
    if buf ~=0
      ...
      fnpl2=fullfile(whereitsat,sprintf('%s-%i-%g.mat','Himalayag',0,buf));
