function varargout=tienshang(res,buf)  
...
if ~isstr(res) % Not a demo
  ...
  if res==0 && buf==0
    fnpl=fullfile(whereitsat,'Tienshang.mat');
  elseif buf==0;
    fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','Tienshang',res));
  elseif buf~=0
    fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','Tienshang',res,buf));
  end
  ...
  if exist(fnpl,'file')==2 
    ...
    if buf==0
        if res==0
          % First part 
          load(fullfile(whereitsat,'Tienshang.mat'));
    ...
    if buf ~=0
      ...
      fnpl2=fullfile(whereitsat,sprintf('%s-%i-%g.mat','Tienshang',0,buf));
