function varargout=wantarcticaGOceanBuf(res,buf)  
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'WantarcticaG.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','WantarcticaG',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','WantarcticaGOceanBuf',res,buf));
...
if exist(fnpl,'file')==2 
  ...
  fnpl2=fullfile(whereitsat,sprintf('%s-%i-%g.mat','WantarcticaGOceanBuf',0,buf));
  if exist(fnpl2,'file')==2
    ...
    [x,y] = polybool('subtraction',XYw(:,1),XYw(:,2),XYeInt(:,1),XYeInt(:,2));   
    [x,y] = polybool('subtraction',x,y,XYp(:,1),XYp(:,2));   
