function varargout=pantarctica2(res,buf)  
% XY=PANTARCTICA(res,buf)
% PANTARCTICA(...) % Only makes a plot
%
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'Pantarctica2.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','Pantarctica2',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','Pantarctica2',res,buf));
end
...
if exist(fnpl,'file')==2 
  ...
  if res==0
    % First part %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    clf
    load(fullfile(getenv('IFILES'),'COASTS','Pantarctica.mat'));
