...
% INPUT:
%
% At the moment there is really only one option available for all these
% inputs, so they are sorta not needed, since we only use 1 type of GLDAS
% product.
% 
% version       What version of GLDAS you want. Currently only [default: GLDAS1]
% gldasres      The spatial resolution of the product you want in degrees. 
%                Currently only [default: 1]
% timeres       The time resolution of the data you want. Choose either:
%                'M' for monthly [default]
%                '3H' for 3-hourly
% forcing       The model forcing used. Currently only [default: 'NOAH']
...
% EXAMPLE: 
%
% See also: READGLDAS, GETGLACIERS, MASKFROMGMT, COARSENMASK
%
...
if masked
fnpl=sprintf('%s/TWS-%s-%s-%s-%s-%s.mat',ddir1,version,forcing,timeres,num2str(gldasres),'masked');
else
fnpl=sprintf('%s/TWS-%s-%s-%s-%s-%s.mat',ddir1,version,forcing,timeres,num2str(gldasres));    
end
