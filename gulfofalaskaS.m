function varargout=gulfofalaskaS(res,buf)  
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'gulfofalaskaS.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','gulfofalaskaS',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','gulfofalaskaS',res,buf));
end
...
if exist(fnpl,'file')==2 
  ...
  if res==0
    % First part %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    clf
    load(fullfile(getenv('IFILES'),'Glaciers','RGI_3_2','gulfofalaskaS.mat'));
...
elseif strcmp(res,'demo1')
      path(path,'~/src/m_map');
      XY1 = gulfofalaskaS(10);
      XY2 = gulfofalaskaS(10,0.5);
      figure
