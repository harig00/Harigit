function varargout=eantarcticaCoasts2OceanBuf(res,buf,rotb)  
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'EantarcticaCoasts2OceanBuf.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','EantarcticaCoasts2OceanBuf',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','EantarcticaCoasts2OceanBuf',res,buf));
end
...
if exist(fnpl,'file')==2 
  ...
  fnpl2=fullfile(whereitsat,sprintf('%s-%i-%g.mat','EantarcticaCoasts2OceanBuf',0,buf));
  if exist(fnpl2,'file')==2
    ...
    XYe2 = eantarcticaCoasts2(res,buf,0);

    % Get the original Interior region
    XYeInt = eantarcticaIntG(0,0,0);
    
    % Now subtract this region
    [x,y] = polybool('subtraction',XYe2(:,1),XYe2(:,2),XYeInt(:,1),XYeInt(:,2));   
