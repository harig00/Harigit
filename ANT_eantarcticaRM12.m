function varargout=ANT_eantarcticaRM12(res,buf)  
...
if ~isstr(res) % Not a demo
  ...
  if res==0 && buf==0
    fnpl=fullfile(whereitsat,'ANT_eantarcticaRM12.mat');
  elseif buf==0;
    fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','ANT_eantarcticaRM12',res));
  elseif buf~=0
    fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','ANT_eantarcticaRM12',res,buf));
  end
  ...
  if exist(fnpl,'file')==2 
    ...
    if buf==0
        if res>10
          ...
          XY=ANT_eantarcticaRM12(10);
          XYb=bezier(XY,round(res/10));
          XY=XYb;
        else
          XY=ANT_eantarcticaRM12(0);
          XYb=bezier(XY,res);
          XY=XYb;
        end
    end
    ...
    if buf ~=0
      ...
      fnpl2=fullfile(whereitsat,sprintf('%s-%i-%g.mat','ANT_eantarcticaRM12',0,buf));
