function varargout=pantarcticaG(res,buf)  
% XY=PANTARCTICAG(res,buf)
% PANTARCTICAG(...) % Only makes a plot
%
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'PantarcticaG.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','PantarcticaG',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','PantarcticaG',res,buf));
end
...
if exist(fnpl,'file')==2 
  ...
  if res==0
    % First part %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    clf
    load(fullfile(getenv('IFILES'),'COASTS','PantarcticaG.mat'));
...
%     % we are on a single hemisphere bookkeeping wise...
%     
