function varargout=eantarcticaInt(res,buf)  
% XY=EANTARCTICAINT(res,buf)
% EANTARCTICAINT(...) % Only makes a plot
%
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'EantarcticaInt.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','EantarcticaInt',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','EantarcticaInt',res,buf));
end
...
if exist(fnpl,'file')==2 
  ...
  load(fullfile(whereitsat,'EantarcticaInt.mat'))
  if res==0
