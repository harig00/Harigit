...
% each date with reference to the first date.  That is, the PGR models are
% saved as yearly rates of surface mass density change, so there will be a 
% different field for each date.
%
% If you want lmcosi fields for PGR, then give only 2 arguments.  If you
% want the PGR projected into a Slepian basis, give that basis.
%
%
% INPUT:
%
...
%                during 2004]
% model        Which PGR model you want.  See references. Options include:
...
% PGRt          If you want SH coefficients for the PGR, then this will be 
%                a 3D matrix of PGR fields for each date requested.  The
...
defval('ddir1',fullfile(IFILES,'PGR',model(1:6)));
else
defval('ddir1',fullfile(IFILES,'PGR',model));
end
...
if exist('TH','var')
    ...
    PGRt(i,:) = falpha*newdates(i);
    if exist('lmcosiU','var') && exist('lmcosiL','var')
        PGRtU(i,:) = falphaU*newdates(i);
        PGRtL(i,:) = falphaL*newdates(i);
    end
        if xver && i==12
   ...
   total=eigfunINT*(falpha(1:round(N))*365); % Back to per year
    ...
    for i = 1:length(newdates)
        ...
        if exist('lmcosiU','var') && exist('lmcosiL','var')
            PGRtU(i,:,:) = [lmcosiU(:,1:2) lmcosiU(:,3:4)*newdates(i)];
            PGRtL(i,:,:) = [lmcosiL(:,1:2) lmcosiL(:,3:4)*newdates(i)];
        end
        if xver && i==2
            plotplm(squeeze(lmcosiPGR(i,:,:)),[],[],4,1)
            pause
        end
    end
