function varargout=wantarcticaCoasts(res,buf)  
% XY=wantarcticaCoasts(res,buf)
% wantarcticaCoasts(...) % Only makes a plot
%
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'WantarcticaCoasts.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','WantarcticaCoasts',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','WantarcticaCoasts',res,buf));
end
