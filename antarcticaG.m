function varargout=antarcticaG(res,buf,rotb)  
...
if res==0 && buf==0
  fnpl=fullfile(whereitsat,'antarcticaG.mat');
elseif buf==0;
  fnpl=fullfile(whereitsat,sprintf('%s-%i.mat','antarcticaG',res));
elseif buf~=0
  fnpl=fullfile(whereitsat,sprintf('%s-%i-%g.mat','antarcticaG',res,buf));
end
...
if exist(fnpl,'file')==2 
  ...
  if res==0
    % First part %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %clf
    load(fullfile(getenv('IFILES'),'COASTS','antarcticaG.mat'));
