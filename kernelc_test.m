function varargout=kernelc_test(Lmax,dom,pars,ngl,rotb)
...
%            'alternative' for an alternative calculation method
...
%          PLOTSLEP, PLM2AVG
%
% Last modified by charig-at-princeton.edu, 02/17/2010
...
if ~isstr(Lmax)
  ...
  if exist(fnpl,'file')==2 && ~isstr(ngl) && Lmax==0
    load(fnpl)
	...
	phint=phint*pi/180;
      ...
      parfor lm1dex=1:dimK
        ...
        %tempstring = strcat('quicksave',num2str(lm1dex));
        %parsave(tempstring,XlmXlmp);
        
        % Do the calculation and set as a temp variable
        %temp3 = (w(:)'*(XlmXlmp(:,bigo(andex:undex)).*I));
        temp3 = (w(:)'*(XlmXlmp.*I));
        % Pad the temp variable with the appropriate zeros out front
        temp3 = [zeros(1,(index-1)) temp3];
